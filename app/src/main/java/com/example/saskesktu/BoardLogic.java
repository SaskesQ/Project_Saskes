...
public class BoardLogic {
    ...
    boolean switched_status; // 1 - Juodos šaškės tampa raudonomis, o Raudonos juodomis, 0 - atvirkščiai
    ...
    public void openDialog() {
        ...
        if (dialog.getFragmentManager() != null) {
            dialog.show(dialog.getFragmentManager(), "Bandymas");
        }
    }

    /**
     * Keičia šaškes, jeigu apkeitimo mygtukas buvo paspaustas
     */
        ...
        if (!pressedStatus) {
            ...
            switch (statusArray[placeId]) {
                case "LightPressed":
                    ...
                    removeHighlightedMoves2(LayoutView, C);
    ...
    public void Kirtimas2(View V, Context C, int sourceKirtimasId, int destKirtimasId, String kirtimoColor) {
        View destination = V.findViewById(checkerIDs[destKirtimasId]);
        View source = V.findViewById(checkerIDs[sourceKirtimasId]);
    ...
    public void removeHighlightedMoves2(View V, Context C) {
        for (int i = 0; i < 64; i++) {
            if (statusArray[i] == "AvailableMove") {
                statusArray[i] = "0";
